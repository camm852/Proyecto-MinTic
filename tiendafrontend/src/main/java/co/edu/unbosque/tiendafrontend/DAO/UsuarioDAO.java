...
public String actualizarUsuario(UsuarioVO user) {
	...
	try {
        ...
        URL url = new URL("http://localhost:8080/consultarUsuario?cedula_usuario="+ user.getCedula_usuario()+"&email_usuario="+user.getEmail_usuario()+"&nombre_usuario="+user.getNombre_usuario()+"&password_usuario="+user.getPassword_usuario()+"&usuario="+user.getUsuario()); //your url i.e fetch data from .
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("GET");
        conn.setRequestProperty("Accept", "application/json");
        if (conn.getResponseCode() != 200) {
